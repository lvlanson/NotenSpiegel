 ...
 * 19.1 MVC-Verfahren popupBroadcast
------------------------------------------------------------------------------------------*/
	/**
...
--------------------------------------------------------*/
	...
	var popupEnableBroadcast = false; // 19.1 MVC
	...
	/**
	 * Popup im Root-Fenster registrieren
	 * @param _name
	 * @param _window
	 */
	 ...
	 * @param self
	 */
	function popupBroadcast(key,value,self)
	{
		if (!popupEnableBroadcast) return;
		
		var root = getRoot()
		if (root && !root.closed)
		{
			if (typeof root.jqContextCallback == "function" && (self || root != window)) root.jqContextCallback(key,value,1); 
			for (var i in root.popupWindows) 
			{
				var popup = root.popupWindows[i];
				if (popup && !popup.closed)
				{
					if (typeof popup.jqContextCallback == "function" && (self || popup != window)) popup.jqContextCallback(key,value,1); 
				}
			}
		}
	}
	...
	// MSIE: Registrierung Popup im Root-Fenster
	if (navigator.userAgent.indexOf("MSIE") != -1 || popupEnableBroadcast)
	{
...
--------------------------------------------------------*/
	function popupOpen(url,name,_width)
	{
		...
		// im MSIE muss man close() auf gleichnamiges Fenster ausführen (siehe oben) 
